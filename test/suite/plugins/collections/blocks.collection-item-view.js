(function() {
	...
	test(
			...
			function() {
				...
				equal(
						view.$el.html(),
						'main <div class="people">The people are loading...</div>',
						'loading template and rendering');
				people.mockFetched();
				people.reset();
				equal(view.$el.html(),
						'main <div class="people">There are no people</div>',
	...
	test("full views for item entries (instead of templates)", function() {
		Blocks.templateEngine = new Blocks.Template.Handlebars();
		Blocks.Defaults.collectionHandlerClass = Blocks.Handler.Collection.ItemView;
		...
		var people = new MockCollection({firstName: 'Joe', lastName: 'Hudson'});
		var view = new MainView();
		view.addCollection('people', people);
		view.render();
		equal(replaceContainerId(view.$el.html()), 'main <div class="people">The items are: <ul class="{*}"><li data-view="{*}">person: Joe Hudson</li></ul></div>');
	});
})();
