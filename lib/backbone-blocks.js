(function(env) {
	var root = env.Blocks = {
		...
		setPageView: function(view, options) {
		...
		template: function(path, view, options) {
			...
			if (!options || !options.content) {
				content = this.getContent(path, view, options);
			}
	...
	_content.ElementContentProvider = _base.extend({
		get : function(path, view, options) {
			...
			if (view) {
				if (view.viewPackage) {
					id += (view.viewPackage.replace(/[\/\.]/, '_') + '_');
				}
				if (view.template) {
					id += _util.getValue(view, 'template');
				}
				else {
					id += view.viewName;
				}
	...
	_.extend(Blocks.ObjectManager.prototype, {
		...
		add : function(type, options) {
			...
			if (!options[type]) {
				throw new Error('Invalid managed object - missing "' + type + '" option value');
			}
