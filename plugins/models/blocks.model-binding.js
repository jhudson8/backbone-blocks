/**
 * Handler & associated field helps to auto bind model values to DOM and DOM changes to model.  Validation errors
...
Blocks.Handler.Field.SimpleFieldHandler = {
	...
	setElementValue: function(key, value, el) {
			var size = el.size();
			if (!size || _.isUndefined(value)) return;
			if (size === 1) {
				// checkbox
...
Blocks.Handler.ModelBinder = Blocks.Handler.Base.extend({
	...
	updateFieldValue: function(key, value) {
		this.fieldHandler(key).setElementValue(key, value, this.fieldMap(key).getElements(key, this.$el));
	},
	...
	inputChanged: function(event) {
		...
		var key = this.fieldMap(key).getFieldKey(element);
